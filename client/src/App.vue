<template>
	<div class="w-screen h-screen bg-stone-300">
		...
		<div v-if="status === 'chat'" class="w-full h-full flex justify-center items-center flex-col gap-4 bg-stone-100">
			...
			<h1 class="font-semibold text-3xl text-emerald-500">Welcome!</h1>
			<p class="text-stone-400 text-lg ">Click the button below to start chatting!</p>
			<button @click="request()" class="bg-emerald-500 rounded-lg text-stone-100 py-2 px-4 text-xl font-semibold">Let's go!</button>
		</div>
		...
		<!-- Chat view -->
		<div v-if="status === 'setup'" class="w-full h-full flex flex-col">
			<!-- Header -->
			...
			<!-- Footer -->
			<footer class="rounded-t-3xl h-24 bg-stone-100 p-4 flex gap-4">
				<input type="text" class="bg-stone-200 rounded-3xl p-1 px-3 text-lg text-stone-700 w-full" placeholder="Type message...">
				<button class="bg-emerald-500 rounded-full p-2 px-3 flex justify-center items-center">
					<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#f5f5f4" class="w-7 h-7">
